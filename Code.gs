...
// Purchase Form (GSTIN, address, insurance, freight, transport quote, etc.)
// that don't belong inside items/steps.
function getOrders() {
...
function saveOrder(order) {
  ...
  const extra = {
    ...
    // Transport quote, as entered on the Purchase Form (planned/quoted -
    // separate from the actual dispatch details confirmed later at PO3).
    transportName: order.transportName, transporterNumber: order.transporterNumber,
    vehicleNumber: order.vehicleNumber, agreedTransportRate: order.agreedTransportRate,
    freightPaidByUs: order.freightPaidByUs, transportPdfLink: order.transportPdfLink
  };
  const rowValues = [
    cleanId,
    order.mill,
...
function backfillFullFields() {
  ...
  // Insurance/Freight (and the rest of the transport quote: Transport Name,
  // Transporter Number, Vehicle Number, Agree Rate of Transport, Freight
  // (Paid by Us), Transport PDF Link, Delivery Date) can land on any one
  // item-row of the shipment (not necessarily the first), and 0 is a
  // legitimate value (not "blank") - so scan every row for that PO and take
  // whichever one actually has a value. One shipment = one value for each of
  // these, no matter how many item rows. Applies to every mill, not just one.
  const formRowsByPo = {};
  for (let i = 1; i < formData.length; i++) {
    const row = formData[i];
    const po = row[col['PO No']] ? String(row[col['PO No']]).trim() : '';
  ...
  const dateOrRaw = v => v instanceof Date ? Utilities.formatDate(v, Session.getScriptTimeZone(), 'yyyy-MM-dd') : v;
  Object.keys(orderMap).forEach(po => {
    ...
    if (!hasVal(o.deliveryDate)) {
      const v = firstNonBlank(rows, col['Delivery Date']);
      if (v !== undefined) o.deliveryDate = dateOrRaw(v);
    }
    if (!hasVal(o.transportName)) {
      const v = firstNonBlank(rows, col['Transport Name']);
      if (v !== undefined) o.transportName = v;
    }
    if (!hasVal(o.transporterNumber)) {
      const v = firstNonBlank(rows, col['Transporter Number']);
      if (v !== undefined) o.transporterNumber = v;
    }
    if (!hasVal(o.vehicleNumber)) {
      const v = firstNonBlank(rows, col['Vehicle Number']);
      if (v !== undefined) o.vehicleNumber = v;
    }
    if (!hasVal(o.agreedTransportRate)) {
      const v = firstNonBlank(rows, col['Agree Rate of Transport']);
      if (v !== undefined) o.agreedTransportRate = v;
    }
    if (!hasVal(o.freightPaidByUs)) {
      const v = firstNonBlank(rows, col['Freight (Paid by Us)']);
      if (v !== undefined) o.freightPaidByUs = v;
    }
    if (!hasVal(o.transportPdfLink)) {
      const v = firstNonBlank(rows, col['Transport PDF Link']);
      if (v !== undefined) o.transportPdfLink = v;
    }
  });
...
function regenerateFlatView() {
  ...
  const header = [
    ...
    'Transport Name (Purchase Form)','Transporter Number (Purchase Form)','Vehicle Number (Purchase Form)',
    'Agreed Transport Rate','Freight Paid By Us','Transport PDF Link'
  ];
  ...
  getOrders().forEach(o => {
    ...
    (o.items || []).forEach(it => {
      rows.push([
        ...
        o.transportName, o.transporterNumber, o.vehicleNumber,
        o.agreedTransportRate, o.freightPaidByUs, o.transportPdfLink
      ]);
    });
  });
